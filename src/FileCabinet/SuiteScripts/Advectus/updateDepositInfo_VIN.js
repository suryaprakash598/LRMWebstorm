...
define(['N/runtime', 'N/task', 'N/record', 'N/search', 'N/log'], function(
    ...
    function afterSubmit(context) {
        try {
            ...
            if (context.type === context.UserEventType.CREATE) {
                ...
                let abpPaymentMethod = rec.getValue({ fieldId: 'custbody_abp_payment_method' }); //CASH: 1 || CHECK : 2 || ACH : 7
                //CHECK DEPOSIT DECLINED ONLY IF CARD IS SELECTED AS PAYMENT METHOD
                if (abpPaymentMethod != '' && abpPaymentMethod != 1 && abpPaymentMethod != 2 && abpPaymentMethod != 7) {
                    var declinedCount = getPaymentDeclined(DepositRecId);
                    if (declinedCount > 0) {
                        return true;
                    }
                }
                if (vin) {
                    ...
                    let customerdepositSearchObj = search.create({
                        ...
                        filters: [
                            ...
                            ["custbody_advs_vin_create_deposit", "anyof", vin], "AND",
                            ["mainline", "is", "T"]
                        ],
                        columns: ["tranid"]
                    });
                    ...
                    } else /*if (SalesQuote == true)*/ { //COMMENTING THIS ON 220525 AS PER FARAZ
                    ...
                    log.debug('DepositRecId',DepositRecId);
                    if (DepositRecId) {
                        Fields['custrecord_advs_lease_inventory_delboard'] = true;
                        log.debug('ReservationStatus',ReservationStatus);
                        ...
                        var vinholdstatusval =0;
                       if(vinholdstatus.custrecord_reservation_hold.length>0){
                            vinholdstatusval = vinholdstatus.custrecord_reservation_hold[0].value;
                       }
                        ...
                        if(Customerid){
                            Fields["custrecord_advs_customer"] = Customerid;
                        }
    ...
    function getDepositBalance(vin) {
        try {
            var customrecord_advs_vm_inv_dep_del_boardSearchObj = search.create({
                type: "customrecord_advs_vm_inv_dep_del_board",
                filters: [
                    ["custrecord_advs_in_dep_vin", "anyof", vin], "AND",
                    ["isinactive", "is", "F"], "AND",
    ...
    function getDeliveryBoardData(VinId) {
        ...
        let SearchObj = search.create({
            ...
            filters: [
                ['isinactive', 'is', 'F'], 'AND',
                ['custrecord_advs_in_dep_balance', 'greaterthan', 0], 'AND',
    ...
    function getSoftHoldData(VinId) {
        ...
        let SearchObj = search.create({
            ...
            columns: ['internalid', 'custrecord_advs_ishlf_registration_fee', 'custrecord_advs_ishlf_title_fee', 'custrecord_advs_ishlf_pickup_fee', 'custrecord_advs_ishlf_deposit_inception', 'custrecord_advs_ishlf_payment_inception', 'custrecord_advs_ishlf_total_inception', 'custrecord_advs_soft_hold_log_sales_quot', 'custrecord_advs_ishlf_truck_ready', 'custrecord_personal_property_tax',
        ...
        DataObj.PickupFee = 0;
        DataObj.DepIncep = 0;
        DataObj.PaymentIncep = 0;
        DataObj.TotIncep = 0;
        DataObj.SalesQuote = 0;
        DataObj.TruckReady = 0;
        DataObj.ppTax = 0;
        SearchObj.run().each(function(result) {
    ...
    function getSoftholdsalesrep(VinId) {
        ...
        let SearchObj = search.create({
            ...
            columns: ['internalid', 'custrecord_advs_soft_hold_salesrep']
        });
        SearchObj.run().each(function(result) {
            let RecordId = result.getValue('internalid');
            let Salesrep = result.getValue('custrecord_advs_soft_hold_salesrep');
    ...
    function getOldValuefromVehicle(vin) {
        let vmSearchObj = search.create({
            type: "customrecord_advs_vm",
            filters: [
                ["systemnotes.field", "anyof", "CUSTRECORD_ADVS_VM_RESERVATION_STATUS"], "AND",
    ...
    function getPaymentDeclined(cdintid) {
        var customrecord_fd_abp_transaction_logSearchObj = search.create({
            type: "customrecord_fd_abp_transaction_log",
            filters: [
                ["custrecord_tl_transaction", "anyof", cdintid], "AND",
                ["custrecord_tl_status", "is", "Declined"]
            ],
            columns: ["custrecord_tl_payment_method", "custrecord_tl_reference_number", "custrecord_tl_auth_code", "custrecord_tl_auth_amount", "custrecord_tl_status_code", "custrecord_tl_command", "custrecord_tl_transaction", "custrecord_tl_saved_card", "custrecord_tl_customer"]
        });
        var searchResultCount = customrecord_fd_abp_transaction_logSearchObj.runPaged().count;
        log.debug("customrecord_fd_abp_transaction_logSearchObj result count", searchResultCount);
        return searchResultCount;
        customrecord_fd_abp_transaction_logSearchObj.run().each(function(result) {
