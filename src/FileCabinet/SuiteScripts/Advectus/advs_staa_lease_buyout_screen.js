...
define(['N/format', 'N/log', 'N/record', 'N/runtime', 'N/search','N/ui/serverWidget','N/error','N/redirect','./advs_lib_rental_leasing','./advs_lib_util'],
	...
	function onRequest(context) {
		...
		if(request.method == "GET"){
			var dealID	=	request.parameters.Custparam_stockID;
			var form	=	serverWidget.createForm({title:"Pay Off"});
...
//			"<script src='https://7402429-sb1.app.netsuite.com/core/media/media.nl?id=9500&c=7402429_SB1&h=Hr13HJj6CWwhXk698LP6dlQ2HvNwWL5zsQPAftRwXFouu9e1&_xt=.js'></script>"+   // from Ohio Sandbox
            "<script src='https://tstdrv1064792.app.netsuite.com/core/media/media.nl?id=129519&c=TSTDRV1064792&h=lsYFL6_Zj6PM4QcpSLm2JQlR-ETkvSnRGWOj-lu1vgTaOUt-&_xt=.js'></script>"+
...
// 					NetPayM	=	NetPayM*1;
				...
				/*	"<div class='form-wrapper'>"+
					...
					"</div>"+
						...
						"<div class='form-wrapper'>"+
						"<label for=''>Pending Scheduled Payment</label>"+
						...
						"</div>"+
						"<div class='form-wrapper'>"+
						"<label for=''>Buyout Offered Value</label>"+
...
//			html_data+=StyleCSS();
			...
			invRec.setValue({fieldId:"custbody_advs_lease_head",value:contId});
				invRec.setValue({fieldId:"custbody_advs_invoice_type",value: setupData[libUtil.rentalinvoiceType.lease_buyout]["id"]});

				log.debug("BUY",libUtil.rentalinvoiceType.lease_buyout);
				log.debug("BUYITEM",setupData[libUtil.rentalinvoiceType.lease_buyout]["buyout"]);
			if(netPayable>0){
				...
				invRec.setCurrentSublistValue({
					sublistId: "item",
					fieldId: "item",
					value:setupData[libUtil.rentalinvoiceType.lease_buyout]["buyout"]
				});
				invRec.setCurrentSublistValue({
					sublistId: "item",
					fieldId: "quantity",
			...
			record.submitFields({
				...
				id:vinID,
				values:{
					"custrecord_advs_vm_reservation_status":libUtil.vmstatus.sold,
